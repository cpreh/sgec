...
{
  FCPPT_NONMOVABLE(sgec_renderer_context_ffp);

public:
  explicit sgec_renderer_context_ffp(sge::renderer::context::ffp_unique_ptr &&);

  ~sgec_renderer_context_ffp();

  void clear(sgec_image_color_rgba);

  [[nodiscard]] sge::renderer::context::ffp &get();
