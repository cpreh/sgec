...
{
  FCPPT_NONMOVABLE(sgec_audio_sound_base);

public:
  explicit sgec_audio_sound_base(sge::audio::sound::base_unique_ptr &&);

  ~sgec_audio_sound_base();

  void play(sgec_audio_sound_repeat);

  [[nodiscard]] sgec_audio_sound_play_status play_status() const;
